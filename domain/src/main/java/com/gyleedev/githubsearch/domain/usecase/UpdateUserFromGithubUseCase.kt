...
/*
    ...
    // Repository는 향후 RemoteMediator를 통해 캐싱 처리하므로 간략하게 캐싱한다.
    // 추가 수정하지 않는다.
    현재 repository 관련 이슈(onConflict -> Replace 상태)

    user 캐싱을 했는데 web에 기록이 없다.
    존재하지 않게 된 유저(탈퇴) 처리 방법을 고민한다.
        - 일단은 예외처리(Exception 으로 Fail 처리)
        - 장기적으로 리팩터링 lastAccess 에 탈퇴정보 추가, 탈퇴면 fetch 하지 않도록
 */
class UpdateUserFromGithubUseCase
    @Inject
    constructor(
        private val repository: GitHubRepository,
        private val clock: Clock,
    ) {
        suspend operator fun invoke(id: String): UserUpdateResult =
            try {
                // 1. AccessTime 을 Local 에서 확인한다.
                ...
                if (lastAccess == null) {
                    ...
                    val durationSinceLastAccess =
                        Instant.now(clock).toEpochMilli() - lastAccess.accessTime.toEpochMilli()
                    ...
                    when {
                        ...
                        isTimeOut -> {
                            ...
                            if (userSync is UserSyncResult.Success) {
                                // repo sync
                                // repo 결과가 없으면 repository 내부에서 insert를 건너뛴다.
