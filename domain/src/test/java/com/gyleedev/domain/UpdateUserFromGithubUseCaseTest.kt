...
class UpdateUserFromGithubUseCaseTest {
    ...
    @Test
    fun `마지막 접근 기록이 없으면 유저를 삭제하고 실패를 반환한다`() =
        runTest {
            // Given
    ...
    @Test
    fun `시간이 만료되었고 유저 싱크가 성공하면 레포 싱크와 시간을 갱신하고 성공을 반환한다`() =
        runTest {
            // Given
            ...
            coEvery { repository.syncUserData(userId) } returns
                UserSyncResult.Success(entityId = expectedEntityId)
    ...
    @Test
    fun `시간이 만료되었으나 유저 싱크가 실패하면 실패를 반환한다`() =
        runTest {
            // Given
            val expectedResult = UserUpdateResult.Fail
            val lastAccess =
    ...
    @Test
    fun `시간이 만료되지 않았으나 레포 싱크가 필요하면 레포를 싱크하고 시간을 갱신하고 성공을 반환한다`() =
        runTest {
            // Given
            val expectedResult = UserUpdateResult.Success
            val expectedEntityId = 1L
    ...
    @Test
    fun `싱크가 모두 완료된 상태이고 시간도 만료되지 않았으면 추가 동작 없이 성공을 반환한다`() =
        runTest {
            // Given
            val expectedResult = UserUpdateResult.Success
            val lastAccess =
    ...
    @Test
    fun `마지막 접근 기록 조회 시 예외가 발생하면 실패를 반환한다`() =
        runTest {
            // Given
            val expectedResult = UserUpdateResult.Fail
    ...
    fun `레포 싱크 시 예외가 발생하면 실패를 반환한다`() =
        runTest {
            ...
            coEvery { repository.syncUserData(userId) } returns
                UserSyncResult.Success(entityId = expectedEntityId)
            coEvery { repository.syncRepoDataList(expectedEntityId, userId) } throws
                RuntimeException("Sync error")
            coEvery { repository.deleteUserById(any()) } just runs
            coEvery { repository.upsertAccessTime(any(), any(), any()) } just runs
            coEvery { repository.getUserId(any()) } returns 0L
    ...
    @Test
    fun `접근 시간 갱신 시 예외가 발생하면 실패를 반환한다`() =
        runTest {
            // Given
            val expectedResult = UserUpdateResult.Fail
            val expectedEntityId = 1L
            ...
            coEvery { repository.syncUserData(userId) } returns
                UserSyncResult.Success(entityId = expectedEntityId)
            coEvery { repository.syncRepoDataList(expectedEntityId, userId) } just runs
            coEvery { repository.upsertAccessTime(expectedEntityId, userId, true) } throws
                RuntimeException("Update error")
            coEvery { repository.deleteUserById(any()) } just runs
            coEvery { repository.getUserId(any()) } returns 0L
    ...
    @Test
    fun `유저 ID 조회 시 예외가 발생하면 실패를 반환한다`() =
        runTest {
            // Given
            val expectedResult = UserUpdateResult.Fail
            val lastAccess =
