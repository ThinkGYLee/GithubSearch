...
class HomeViewModelTest {
    ...
    @Test
    fun `초기화 시 유저 리스트를 가져온다`() = runTest {
        // Given
    ...
    @Test
    fun `검색어 입력 후 300ms가 지나기 전에는 검색 API가 호출되지 않는다`() = runTest {
        // Given
        val query = "test"
    ...
    @Test
    fun `검색어 입력 후 300ms가 지나면 검색 API가 호출되고 결과가 반영된다`() = runTest {
        // Given
        val query = "test"
        val expectedUser = createDummyUser(query)
        val expectedSearchState = SearchUiState.Success(
    ...
    @Test
    fun `검색어가 빈 칸이면 API를 호출하지 않고 상태가 Empty가 된다`() = runTest {
        // Given
        val query = "  "
        val expectedUiState = HomeUiState.Success(
            searchQuery = query,
    ...
    @Test
    fun `검색 요청이 성공하면 로딩 상태가 true가 되었다가 완료 후 false가 된다`() = runTest {
        // Given
        // 테스트에 사용할 검색어 정의
        val query = "test"
        // 변화하는 UI 상태들을 순서대로 저장할 리스트 생성
        ...
        // UI 상태가 변할 때마다 uiStates 리스트에 자동으로 저장하도록 설정 (비동기 수집 시작)
        viewModel.uiState.collectIn(backgroundScope, uiStates)
        ...
        val loadingState = uiStates.filterIsInstance<HomeUiState.Success>().find { it.isLoading }
        // 로딩 상태가 실제로 존재했는지 검증
        ...
        val finalState = uiStates.last() as HomeUiState.Success
        // 최종적으로 로딩 상태가 해제되었는지 검증
        assertEquals(false, finalState.isLoading)
    }

    @Test
    fun `검색 요청 시 인증이 필요하면 다이얼로그 상태가 true가 된다`() = runTest {
        // Given
        val query = "test"
