...
class FavoriteViewModelTest {
    ...
    @Test
    fun `초기화 시 기본 상태가 ALL이며 ALL 필터로 아이템을 요청한다`() =
        runTest {
            // Given
            val expectedUiState =
    ...
    @Test
    fun `필터를 업데이트하면 상태가 변경되고 해당 필터로 아이템을 요청한다`() =
        runTest {
            // Given
    ...
    @Test
    fun `즐겨찾기 다이얼로그를 띄우면 상태가 반전되고 타겟 유저가 저장된다`() =
        runTest {
            // Given
    ...
    @Test
    fun `즐겨찾기 상태를 업데이트하면 저장된 유저로 UseCase를 호출하고 다이얼로그를 닫는다`() =
        runTest {
            // Given
            val targetUser = createDummyUser("test")
