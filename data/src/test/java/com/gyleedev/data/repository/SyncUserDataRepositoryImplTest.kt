...
class SyncUserDataRepositoryImplTest {
    ...
    @Test
    fun `유저 데이터 동기화 시 API 호출이 성공하고 로컬 유저가 존재하면 즐겨찾기 상태를 유지하며 DB를 업데이트한다`() =
        runTest {
            // Given
            ...
            val localUserEntity: UserEntity =
                createDummyUserEntity(id = 1L, repoCount = 5).copy(
                    githubId = githubId,
                    favorite = true,
                )

            val expectedMergedEntity: UserEntity =
                remoteUserResponse
                    .toModel()
                    .copy(
    ...
    @Test
    fun `유저 데이터 동기화 시 API 호출은 성공했으나 로컬 유저가 존재하지 않으면 실패를 반환한다`() =
        runTest {
            // Given
            val githubId = "test_user"
            val remoteUserResponse =
    ...
    @Test
    fun `유저 데이터 동기화 시 API 호출이 실패하면 로컬 유저 조회 후 Fail을 반환하고 저장은 수행하지 않는다`() =
        runTest {
            // Given
            val githubId = "test_user"
            val expectedResult = UserSyncResult.Fail
