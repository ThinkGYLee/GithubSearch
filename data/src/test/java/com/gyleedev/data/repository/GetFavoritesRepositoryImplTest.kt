...
class GetFavoritesRepositoryImplTest {
    ...
    // 10개의 고정된 더미 데이터 리스트
    private val fullUserList = listOf(
    ...
    @Test
    fun `즐겨찾기 목록 조회 시 FilterStatus가 ALL이면 10개의 모든 유저 데이터를 반환한다`() = runTest {
        // Given
    ...
    @Test
    fun `즐겨찾기 목록 조회 시 FilterStatus가 REPO이면 레포지토리가 있는 유저들만 반환한다`() = runTest {
        // Given
    ...
    @Test
    fun `즐겨찾기 목록 조회 시 FilterStatus가 NOREPO이면 레포지토리가 없는 유저들만 반환한다`() = runTest {
        // Given
    ...
    @Test
    fun `즐겨찾기 목록이 비어있을 때 빈 리스트를 정상적으로 반환한다`() = runTest {
        // Given
        val expectedStatus = FilterStatus.ALL
